---
name: release
description: Cut a PyPI release of physure and physure-core. Use when asked to release, publish, bump the version, or tag a version — the tag/version match is enforced by CI and there are two independent version numbers.
---
...
1. `.github/workflows/bump-release.yml` (manual trigger) — bumps
   `__version__` in `physure/__init__.py` by the requested part
   (patch/minor/major), bumps `physure_core/Cargo.toml` too if
   `physure_core/src/` changed since the last tag, commits, tags
   `vX.Y.Z`, and pushes.
2. `.github/workflows/release.yml` (triggered by the `v*` tag push) — builds
   and publishes **both** packages (`physure` pure wheel + sdist,
   `physure-core` abi3-py310 wheels + sdist) to PyPI via Trusted Publishing
...
- `physure` version: `__version__` in `physure/__init__.py`
  (pyproject uses `dynamic = ["version"]`). **CI rejects the tag if it doesn't
  match `__version__` exactly.**
- `physure-core` version: `version` in `physure_core/Cargo.toml`.
...
2. Trigger the bump (choose the semver part — `physure-core`'s bump, if
...
4. Watch the release run: `gh run watch` (jobs: check-version → core-wheels /
   core-sdist / physure-dist → publish). The publish job has two separate
   upload steps because Trusted Publishing mints per-project tokens.
5. Verify: `pip index versions physure` or check https://pypi.org/project/physure/.
