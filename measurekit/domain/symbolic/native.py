# measurekit/domain/symbolic/native.py
"""Pure-Python fallback for the native symbolic-math AST (`measurekit_core.Expr`).

Mirrors `measurekit_core/src/symbolic.rs` node-for-node: build, simplify, and
...
engine when `measurekit_core` is importable, else falls back to `PyExpr`.
"""
...
class PyExpr:
    """Pure-Python mirror of `measurekit_core.Expr`'s public surface."""
