...
def test_correlated_arrays():
    ...
    # Verify cross-correlation: Cov(q1, q2) = Cov(q_base, 2*q_base) = 2*Var(q_base)
    ...
    print(
        "Success: Cross-covariance Cov(q1, q2) matches theoretical 2 * Var(q_base)."
    )
    ...
    # 2 * Cov(q1, q2) = 2 * (2 * Var(q_base)) = 4 * Var(q_base) = [0.04, 0.16]
    # Total Var(q3) = [0.01+0.04+0.04, 0.04+0.16+0.16] = [0.09, 0.36]
