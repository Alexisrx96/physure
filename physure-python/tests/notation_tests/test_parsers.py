...
def test_parse_simple_units():
    ...
    assert parse_unit_expression("m·s").dimensions == {"m": (1, 1), "s": (1, 1)}
...
def test_parse_complex_expressions():
    ...
    assert parse_unit_expression("m/s").dimensions == {"m": (1, 1), "s": (-1, 1)}
    assert parse_unit_expression("kg·m/s²").dimensions == {"kg": (1, 1), "m": (1, 1), "s": (-2, 1)}
    assert parse_unit_expression("kg*m^2/s^2").dimensions == {"kg": (1, 1), "m": (2, 1), "s": (-2, 1)}
...
def test_parse_with_parentheses():
    ...
    assert parse_unit_expression("(m)").dimensions == {"m": (1, 1)}
    assert parse_unit_expression("(m/s)").dimensions == {"m": (1, 1), "s": (-1, 1)}
    assert parse_unit_expression("(m/s)²").dimensions == {"m": (2, 1), "s": (-2, 1)}
    assert parse_unit_expression("(m·(kg/s))").dimensions == {"kg": (1, 1), "m": (1, 1), "s": (-1, 1)}
...
def test_parse_with_unity():
    ...
    assert parse_unit_expression("kg·1/s").dimensions == {"kg": (1, 1), "s": (-1, 1)}
