...
def native_lib_dir():
    ...
    if not os.path.exists(os.path.join(target_dir, "libphysure_java.so")):
        ...
        assert build.returncode == 0, (
            f"Could not build the JNI bindings: {build.stderr}"
        )
    return target_dir
...
def test_java_transpiler_parity(name, script, expected_vars):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        java_src_dir = os.path.join(
            REPO_ROOT, "physure-java", "src", "main", "java"
        )
        ...
        assert compile_base.returncode == 0, (
            f"Base Java compilation failed: {compile_base.stderr}"
        )
        ...
        assert compile_gen.returncode == 0, (
            f"Generated Java compilation failed: {compile_gen.stderr}\nCode:\n{java_code}"
        )
        ...
        for expected in expected_vars.values():
            assert expected in out, (
                f"Expected {expected} in Java stdout:\n{out}"
            )
...
''')
        ...
        assert proc.returncode == 0, (
            f"Rust compilation/execution failed: {proc.stderr}\nCode:\n{rust_code}"
        )
        out = proc.stdout
        for expected in expected_vars.values():
            assert expected in out, (
                f"Expected {expected} in Rust stdout:\n{out}"
            )
