...
def test_autograd_propagation():
    ...
    # The uncertainties below are live torch tensors: their coefficients carry an
    # autograd graph, which the Rust core cannot hold, so this is the case that has
    # to ask for the Python lineage implementation explicitly.
    try:
        with physure.python_lineage():
            _run_autograd_propagation()
    finally:
        _current_store.reset(token)


def _run_autograd_propagation():
