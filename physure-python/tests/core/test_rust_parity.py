...
def test_config_registry_parity():
    ...
    val, _desc, _latex = constants_meta["speed_of_light_in_vacuum"]
...
def test_phy_function_parity():
    ...
    from physure import Q_, Interpreter, PhyFunction

    interp = Interpreter()

    # 1. Register function
    ke = PhyFunction(
        interp, "kinetic_energy", "kinetic_energy(m, v) = 0.5 * m * v^2"
    )
