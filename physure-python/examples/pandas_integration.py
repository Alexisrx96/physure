...
mags = df["kV"].phs.magnitude
uncs = df["kV"].phs.uncertainty

print("\n--- Extracted Data ---")
print(f"Magnitudes: \n{mags}")
print(f"Uncertainties: \n{uncs}")
print(f"Shared Unit: {df['kV'].phs.unit}")
...
df["volts_err"] = df["volts"].phs.plus_minus(df["volts"].phs.magnitude * 0.05)
...
# df["A"].phs.array returns a single Quantity object backed by a NumPy array.
v_array = df["volts"].phs.array
...
json_output = df["volts_err"].phs.to_json()
print("\n--- JSON Serialization ---")
print(json_output)
